...
const STATUS_LABELS = { draft: 'Borrador', published: 'Publicado', paused: 'Pausado', disabled: 'Deshabilitado' };
...
function StateMessage({ error, onRetry }) {
  return (
    <div className={`ecom-admin-state ${error ? 'is-error' : ''}`} role={error ? 'alert' : 'status'}>
      ...
      {error && <button type="button" className="btn btn-secondary" onClick={onRetry}><RefreshCw size={16} /> Reintentar</button>}
    </div>
  );
}
...
export default function EcommercePortalSettings() {
  ...
  const [features, setFeatures] = useState({ customSlug: false, maxPublishedProducts: 10 });
  ...
  const linkedRefs = useMemo(() => new Set(products.map((item) => item.localProductRef).filter(Boolean)), [products]);
  ...
  const load = useCallback(async () => {
    ...
    setFeatures(result.features || { customSlug: false, maxPublishedProducts: 10 });
  ...
  const loadLocalCatalog = async () => {
    ...
    try {
      ...
      setLocalProducts((page?.data || []).filter((product) => product?.id && product.isActive !== false));
    ...
    <div className="ecom-admin-page">
      <header className="ecom-admin-hero">
        <div>
          ...
          <p>Configura lo que veran tus clientes. La ruta publica y la recepcion de pedidos se habilitaran en fases posteriores.</p>
        </div>
      ...
      {!portal ? (
        <section className="ui-card ecom-admin-empty-card">
          ...
          <div>
            ...
            <p>Se creara en borrador usando los datos guardados. La tienda publica todavia no se habilitara.</p>
          </div>
      ...
      <form className="ui-card ecom-admin-form-card" onSubmit={submitPortal}>
        <div className="ecom-admin-card-heading">
          <div><span className="ecom-admin-eyebrow">Datos publicos basicos</span><h3>Informacion de tu tienda</h3><p>Estos datos se mostraran cuando se active la ruta publica.</p></div>
          <Save size={22} />
        </div>
        <div className="ecom-admin-form-grid">
          ...
          <label className="form-group">
            ...
            <div className="ecom-admin-input-icon"><Link2 size={16} /><input className="form-input" value={form.slug} onChange={updateForm('slug')} minLength={3} maxLength={64} placeholder={isPro ? 'mi-negocio' : 'Generado por el sistema'} readOnly={!isPro} disabled={!isPro} /></div>
            <small className="ecom-admin-help">{isPro ? 'En Lanzo Nube puedes personalizar el enlace de tu tienda.' : 'En Plan Free el enlace se genera automaticamente.'}</small>
          </label>
          ...
          <label className="form-group"><span className="form-label">WhatsApp</span><input className="form-input" type="tel" value={form.whatsappPhone} onChange={updateForm('whatsappPhone')} placeholder="961 000 0000" /><small className="ecom-admin-help">Minimo 8 digitos si agregas un numero.</small></label>
          ...
          <fieldset className="ecom-admin-delivery ecom-admin-span-2">
            ...
            <label><input type="checkbox" checked={form.pickupEnabled} onChange={updateForm('pickupEnabled')} /><span><strong>Recoger</strong><small>El cliente recoge en el negocio.</small></span></label>
            <label><input type="checkbox" checked={form.deliveryEnabled} onChange={updateForm('deliveryEnabled')} /><span><strong>Domicilio</strong><small>El negocio coordina la entrega.</small></span></label>
          </fieldset>
          ...
          <div className="form-group"><span className="form-label">Logo reutilizado</span><div className="ecom-admin-logo">{publicUrl(form.logoUrl || companyProfile?.logo) ? <img src={publicUrl(form.logoUrl || companyProfile?.logo)} alt="Logo del portal" /> : <ImageIcon size={28} />}<span>Se usa el logo ya configurado. Esta fase no agrega nuevas subidas.</span></div></div>
        </div>
        <div className="ecom-admin-customization"><div><Palette size={20} /><span><strong>{isPro ? 'Personalizacion Portal PRO' : 'Plantilla fija de Plan Free'}</strong><small>{isPro ? 'Logo, portada, color principal y plantilla.' : 'Configuracion basica con plantilla fija.'}</small></span></div><span className="ecom-admin-locked"><Lock size={14} /> Disponible en una fase posterior de Portal PRO.</span></div>
      ...
      <section className="ui-card ecom-admin-products-card">
        <div className="ecom-admin-card-heading">
          <div><span className="ecom-admin-eyebrow">Catalogo publico</span><h3>Productos publicados en portal</h3><p>{isPro ? `${publishedCount} productos publicados` : `${publishedCount} / ${maxProducts} productos publicados`}</p></div>
        ...
        {!isPro && <div className={`ecom-admin-limit ${limitReached ? 'is-blocked' : ''}`}><Lock size={17} /> Plan Free permite publicar hasta 10 productos. Actualiza a Lanzo Nube para productos ilimitados.</div>}
        {products.length === 0 ? (
          ...
          <div className="ecom-admin-product-list">
            {products.map((product) => (
              <article key={product.id} className={`ecom-admin-product ${product.isPublished ? '' : 'is-hidden'}`}>
                <span className="ecom-admin-product-image">{product.imageUrl ? <img src={product.imageUrl} alt="" /> : <Store size={22} />}</span>
                <div><div><strong>{product.publicName}</strong><span className={`ecom-admin-mini-status ${product.isPublished ? 'is-on' : ''}`}>{product.isPublished ? 'Publicado' : 'Oculto'}</span></div><span>{product.categoryName || 'Sin categoria'} · ${numberOr(product.price).toFixed(2)}</span><small>{product.isAvailable ? 'Disponible' : 'No disponible'} · Orden {product.displayOrder || 0}</small></div>
                <div className="ecom-admin-product-actions"><button type="button" className="ecom-admin-icon-button" onClick={() => openEditProduct(product)} title="Editar"><Pencil size={18} /></button><button type="button" className="ecom-admin-icon-button" onClick={() => toggleProduct(product)} disabled={busyProductId === product.id} title={product.isPublished ? 'Despublicar' : 'Publicar'}>{busyProductId === product.id ? <LoaderCircle className="ecom-admin-spin" size={18} /> : product.isPublished ? <EyeOff size={18} /> : <Eye size={18} />}</button></div>
              </article>
            ))}
          </div>
        )}
