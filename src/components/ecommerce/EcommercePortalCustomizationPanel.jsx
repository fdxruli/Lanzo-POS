...
export default function EcommercePortalCustomizationPanel({
  ...
  return (
    <section className="ecom-customization" aria-label="Personalización visual">
      <div className="ecom-customization__heading"><div><h3>Personaliza tu tienda</h3><p>Usa Vista previa para revisar el borrador antes de guardarlo.</p></div><button type="button" className="btn btn-secondary" onClick={reset} disabled={disabled || busy}><RotateCcw size={16} /> Restablecer</button></div>
      <div className="ecom-customization__grid">
        <div>
          <fieldset className="ecom-customization__section ecom-customization__section--template"><legend>Plantilla</legend><div className="ecom-template-options">{ECOMMERCE_PORTAL_TEMPLATES.map((code) => <button key={code} type="button" aria-pressed={templateCode === code} className={`ecom-template-option ${templateCode === code ? 'is-selected' : ''}`} onClick={() => setTemplateCode(code)} disabled={disabled || busy}><span className={`ecom-template-option__art ecom-template-option__art--${code}`} /><strong>{TEMPLATES[code][0]}</strong><small>{TEMPLATES[code][1]}</small></button>)}</div></fieldset>
          <fieldset className="ecom-customization__section ecom-customization__section--colors"><legend>Colores</legend>{['primaryColor', 'secondaryColor'].map((field) => <label className="ecom-color-field" key={field}><span>{field === 'primaryColor' ? 'Color principal' : 'Color secundario'}</span><input type="color" value={isEcommercePortalHexColor(theme[field]) ? theme[field] : '#000000'} onChange={(event) => changeColor(field, event.target.value)} disabled={disabled || busy} /><input value={theme[field]} maxLength={7} aria-invalid={!isEcommercePortalHexColor(theme[field])} onChange={(event) => changeColor(field, event.target.value)} disabled={disabled || busy} />{!isEcommercePortalHexColor(theme[field]) ? <small role="alert">Usa un color hexadecimal como #0284c7.</small> : null}</label>)}</fieldset>
          <fieldset className="ecom-customization__section ecom-customization__section--corners"><legend>Esquinas</legend><div className="ecom-choice-row">{ECOMMERCE_PORTAL_CORNER_STYLES.map((value) => <button key={value} type="button" aria-pressed={theme.cornerStyle === value} onClick={() => setTheme((current) => ({ ...current, cornerStyle: value }))} disabled={disabled || busy}>{CORNERS[value]}</button>)}</div></fieldset>
          <fieldset className="ecom-customization__section ecom-customization__section--typography"><legend>Tipografía</legend><div className="ecom-choice-row">{ECOMMERCE_PORTAL_FONT_STYLES.map((value) => <button key={value} type="button" aria-pressed={theme.fontStyle === value} onClick={() => setTheme((current) => ({ ...current, fontStyle: value }))} disabled={disabled || busy}>{FONTS[value]}</button>)}</div></fieldset>
          <fieldset className="ecom-customization__section ecom-customization__section--images"><legend>Imágenes</legend><div className="ecom-image-actions">{imageControls.map(([label, image, setImage, purpose]) => <div key={label} className="ecom-image-action">{image.value ? <img src={image.value} alt={`${label} actual`} /> : <ImagePlus aria-hidden="true" />}<strong>{label}</strong><label className="btn btn-secondary"><UploadCloud size={16} /> Cambiar<input type="file" accept="image/jpeg,image/png,image/webp,image/gif" onChange={(event) => upload({ file: event.target.files?.[0], purpose, image, setImage })} disabled={disabled || busy} /></label>{image.value ? <button type="button" className="btn btn-secondary" onClick={() => clearImage(setImage)} disabled={disabled || busy}><Trash2 size={16} /> Desvincular</button> : null}</div>)}</div></fieldset>
        </div>
      </div>
    </section>
  );
