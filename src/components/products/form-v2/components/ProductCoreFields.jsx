...
export default function ProductCoreFields({ values, errors, onFieldChange, onCostChange, onPriceChange, onMarginChange, onScan }) {
  return <section className="product-form-v2__core" aria-label="Datos principales">
    ...
    <div className="product-form-v2__pricing">
      ...
      <div className="product-form-v2__field"><label htmlFor="product-v2-margin">Margen</label><div className="product-form-v2__suffix"><input id="product-v2-margin" type="number" min="0" step="0.1" value={values.margin} onChange={(event) => onMarginChange(event.target.value)} /><span>%</span></div></div>
      <div className="product-form-v2__field"><label htmlFor="product-v2-price">Precio de venta <span aria-hidden="true">*</span></label><input id="product-v2-price" type="number" min="0" step="0.01" value={values.price} onChange={(event) => onPriceChange(event.target.value)} aria-invalid={Boolean(errors.price)} /><Error>{errors.price}</Error></div>
    </div>
  </section>;
}
