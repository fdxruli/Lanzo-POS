export default function ProductInventoryFields({ values, errors, onTrackStock, onFieldChange, isEditing = false }) {
  return <section className="product-form-v2__inventory" aria-label="Inventario inicial">
    ...
    {values.trackStock && <div className="product-form-v2__inventory-fields">
      <div className="product-form-v2__field"><label htmlFor="product-v2-stock">{isEditing ? 'Existencia actual' : 'Existencia inicial'}</label>{isEditing ? <><output id="product-v2-stock">{values.stock}</output><small className="product-form-v2__help">Las nuevas entradas se administran mediante existencias o lotes.</small></> : <><input id="product-v2-stock" type="number" min="0" step="any" value={values.stock} onChange={(event) => onFieldChange('stock', event.target.value)} aria-invalid={Boolean(errors.stock)} />{errors.stock && <small className="product-form-v2__error">{errors.stock}</small>}</>}</div>
