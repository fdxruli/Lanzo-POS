...
export default function CloudKitchenMonitorRest8ContainerFixed({ kitchenCloud }) {
  const handleAdvanceStatus = async (order) => {
    ...
    for (const item of getItems(order).filter((entry) => getItemId(entry) && shouldAdvanceItem(entry, action.nextStatus))) {
      const result = await kitchenCloud.changeOrderItemStatus({ restaurantOrderId: order.id, restaurantOrderItemId: getItemId(item), status: action.nextStatus });
    ...
    if (result?.success === false) showMessageModal(result.message || kitchenCloud.error || 'No pudimos actualizar la comanda.', null, { type: 'error' });
  };
  ...
  const handleItemStatusChange = async (order, item, status) => {
    ...
    if (result?.success === false) showMessageModal(result.message || kitchenCloud.error || 'No pudimos actualizar el item.', null, { type: 'error' });
  };
  ...
  const handleCancelItemStatus = async (order, item) => {
    ...
    if (!(await showConfirmModal('¿Cancelar este item en cocina? El cajero deberá revisar la cuenta si aplica.', { title: 'Cancelar item', confirmButtonText: 'Sí, cancelar item', cancelButtonText: 'Volver' }))) return;
    ...
    if (result?.success === false) showMessageModal(result.message || kitchenCloud.error || 'No pudimos cancelar el item.', null, { type: 'error' });
    else showMessageModal('Item cancelado en cocina. Recuerda revisar la cuenta si aplica.', null, { type: 'success' });
  };

  const handleCancelOrder = async (order) => {
    if (!(await showConfirmModal('¿Cancelar esta comanda en cocina?', { title: 'Cancelar comanda', confirmButtonText: 'Sí, cancelar', cancelButtonText: 'Volver' }))) return;
    const result = await kitchenCloud.changeOrderStatus({ restaurantOrderId: order.id, status: 'cancelled' });
    if (result?.success === false) showMessageModal(result.message || kitchenCloud.error || 'No pudimos cancelar la comanda.', null, { type: 'error' });
    else showMessageModal('Comanda cancelada en cocina', null, { type: 'success' });
  };
  ...
  const handleArchiveOrder = async (order) => {
    ...
    if (!(await showConfirmModal('Esta acción solo ocultará la comanda de vistas activas. No borra ventas, caja ni inventario.', { title: 'Archivar comanda', confirmButtonText: 'Archivar comanda', cancelButtonText: 'Volver' }))) return;
  ...
  return <CloudKitchenMonitorRest8 kitchenCloud={kitchenCloud} onAdvanceStatus={handleAdvanceStatus} onCancelOrder={handleCancelOrder} onChangeItemStatus={handleItemStatusChange} onCancelItemStatus={handleCancelItemStatus} onArchiveOrder={handleArchiveOrder} />;
}
