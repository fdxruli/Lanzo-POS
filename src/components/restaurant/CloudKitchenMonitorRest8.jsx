...
export default function CloudKitchenMonitorRest8({ kitchenCloud, onAdvanceStatus, onCancelOrder, onChangeItemStatus, onCancelItemStatus, onArchiveOrder }) {
  ...
  return <>
    ...
    {Boolean(productionSummary?.length) && <section className="production-bar"><div className="prod-heading"><span>Producción</span><strong>Ahora</strong></div><div className="prod-list">{productionSummary.map(([name, count]) => <div key={name} className="prod-badge"><span className="prod-count">{count}</span><span className="prod-name">{name}</span></div>)}</div></section>}
    <div className="kds-grid">
      ...
      {displayedOrders.map((order) => {
        ...
        return <div key={order.id || order.localOrderId || order.saleId} className={`kds-ticket status-${displayStatus} ${archived ? 'is-archived' : ''}`}>
          ...
          <div className="ticket-footer"><div className="ticket-meta cloud"><span>Creada {formatTime(order.createdAt || order.timestamp)}</span><span>Actualizada {formatTime(order.updatedAt || order.createdAt || order.timestamp)}</span>{Boolean(total) && <span>Total ref. {total}</span>}</div><div className="ticket-actions">{!isTerminal && !isHistory && hasWritePermission && <button type="button" className="btn-kds-cancel" onClick={() => onCancelOrder(order)} disabled={isUpdating}><X size={18} /></button>}{actionLabel && hasWritePermission && <button type="button" className={`btn-kds-action ${displayStatus === 'ready' ? 'deliver' : 'advance'}`} onClick={() => onAdvanceStatus(order)} disabled={isUpdating}>{updatingOrderId === order.id ? 'Actualizando...' : actionLabel}</button>}{canArchive && <button type="button" className="btn-kds-archive" onClick={() => onArchiveOrder(order)} disabled={isUpdating || archivingOrderId === order.id}><Archive size={16} />{archivingOrderId === order.id ? 'Archivando...' : 'Archivar'}</button>}</div></div>
        </div>;
      })}
    </div>
  </>;
