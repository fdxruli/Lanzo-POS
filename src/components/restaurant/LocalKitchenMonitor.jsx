  ...
  <section className="kds-command-center mode-local">
    ...
    <div className="kds-tabs" role="tablist" aria-label="Estados de cocina">
      {tabs.map(({ key, label, count, Icon, tone, active, onClick }) => (
        <button key={key} type="button" className={`kds-tab tone-${tone} ${active ? 'active' : ''}`} onClick={onClick} aria-pressed={active}>
          <Icon size={18} />
          <span>{label}</span>
          <strong>{count}</strong>
        </button>
...
const ProductionSummary = ({ items }) => {
  ...
  return (
    <section className="production-bar" aria-label="Resumen a producir">
      <div className="prod-heading"><span>Producción</span><strong>Ahora</strong></div>
...
export default function LocalKitchenMonitor() {
  ...
  const playNotificationSound = useCallback(() => {
    if (audioPlayer.current) audioPlayer.current.play().catch(() => Logger.info('Interacción requerida para audio'));
  }, []);
  ...
  const productionSummary = useMemo(() => {
    ...
    const summary = {};
    displayedOrders.forEach((order) => getOrderItems(order).forEach((item) => {
  ...
  const localTabs = [
    { key: 'pending', label: 'Cocina', count: localStatusCounts.pending, Icon: Flame, tone: 'pending', active: filter === 'pending', onClick: () => setFilter('pending') },
    { key: 'ready', label: 'Entrega', count: localStatusCounts.ready, Icon: CheckCircle, tone: 'ready', active: filter === 'ready', onClick: () => setFilter('ready') },
    { key: 'history', label: 'Historial', count: localStatusCounts.history, Icon: History, tone: 'history', active: filter === 'history', onClick: () => setFilter('history') }
  ];

  return (
    <div className="kds-container mode-local">
      <audio ref={audioPlayer} src={NOTIFICATION_SOUND} />
      <KdsCommandCenter metrics={localMetrics} tabs={localTabs} onRefresh={fetchOrders} isLoading={isLoading} />
      <ProductionSummary items={productionSummary} />
      <div className="kds-grid">
        {isLoading && <div className="kds-loading">Conectando con meseros...</div>}
        {!isLoading && displayedOrders.length === 0 && (
          <div className="kds-empty">
            <div className="empty-icon">{filter === 'pending' ? <ChefHat size={64} /> : filter === 'ready' ? <CheckCircle size={64} /> : <History size={64} />}</div>
            <h3>{filter === 'pending' ? 'Cocina despejada' : filter === 'ready' ? 'Todo entregado' : 'Sin historial reciente'}</h3>
            <p>{filter === 'pending' ? 'Esperando nuevas comandas...' : filter === 'ready' ? 'No hay pedidos esperando entrega.' : 'Las comandas de las últimas 24h aparecerán aquí.'}</p>
          </div>
        )}
        {displayedOrders.map((order) => {
          ...
          return (
            <div key={order.id || order.timestamp} className={`kds-ticket status-${displayStatus}`} role="article">
              <div className="ticket-header">
                <div className="ticket-info">
                  ...
                  <span className="ticket-customer">
                    {order.tableData ? <><UtensilsCrossed size={14} /> {order.tableData}</> : order.customerId ? <><User size={14} /> Cliente</> : <><Store size={14} /> Mostrador</>}
                  </span>
                </div>
              ...
              <div className="ticket-progress-row"><span>{ticketItems.length} productos</span><span>{filter === 'pending' ? 'Preparar' : filter === 'ready' ? 'Entregar' : STATUS_LABELS[displayStatus] || displayStatus}</span></div>
                ...
                {ticketItems.map((item, idx) => {
                  ...
                  return (
                    <div key={idx} className="ticket-item">
                      <div className="item-main"><span className="item-qty">{getItemQuantity(item)}</span><span className="item-name">{getItemName(item)}</span></div>
                      {modifiers.length > 0 && <div className="item-modifiers">{modifiers.map((name, i) => <span key={`${name}-${i}`} className="modifier-tag">{name}</span>)}</div>}
                      {item.notes && <div className="item-note"><StickyNote size={14} /><span>{item.notes}</span></div>}
                    </div>
                  );
                })}
              </div>
              <div className="ticket-footer">
                <div className="ticket-meta">{new Date(order.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div>
                <div className="ticket-actions">
                  {filter === 'pending' && <><button type="button" className="btn-kds-cancel" onClick={() => handleCancelOrder(order)} title="Rechazar" aria-label="Rechazar comanda"><X size={18} /></button><button type="button" className="btn-kds-action advance" onClick={() => handleAdvanceStatus(order)}>¡LISTO!</button></>}
                  {filter === 'ready' && <button type="button" className="btn-kds-action deliver" onClick={() => handleAdvanceStatus(order)}>ENTREGADO</button>}
                </div>
              </div>
            </div>
          );
