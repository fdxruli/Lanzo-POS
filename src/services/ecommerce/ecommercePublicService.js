...
function withTimeout(promise, { timeoutMs = PUBLIC_RPC_TIMEOUT_MS, timeoutMessage = STORE_REQUEST_MESSAGE } = {}) {
  let timeoutId;
...
function getSafeMessage(code, operation) {
  ...
  if (code === 'ECOMMERCE_CATALOG_REVISION_CHANGED') return 'El catálogo cambió mientras se cargaba. Se actualizará automáticamente.';
...
function normalizeRpcFailure(data, error, operation = 'store') {
  if (error) {
    return new EcommercePublicError(
      ...
      operation === 'checkout' ? CHECKOUT_REQUEST_MESSAGE
        : operation === 'configuration' ? CONFIGURATION_REQUEST_MESSAGE : STORE_REQUEST_MESSAGE,
      error
    );
  }
...
function normalizeFeatures(rawFeatures) {
  ...
  return {
    ...
    brandingCustomization: typeof features.brandingCustomization === 'boolean'
      ? features.brandingCustomization : asText(features.brandingCustomization, 'basic'),
    layoutCustomization: typeof features.layoutCustomization === 'boolean'
      ? features.layoutCustomization : asText(features.layoutCustomization, 'template_only'),
    businessHours: asBoolean(features.businessHours, true),
    deliveryPickupSettings: typeof features.deliveryPickupSettings === 'boolean'
      ? features.deliveryPickupSettings : asText(features.deliveryPickupSettings, 'basic'),
...
const AVAILABILITY_CODES = new Set([
  'OPEN','ORDERING_DISABLED','ORDERS_PAUSED','OUTSIDE_BUSINESS_HOURS',
  'SCHEDULE_NOT_CONFIGURED','PORTAL_NOT_PUBLISHED'
]);
...
function normalizeAvailability(rawAvailability, portal, isPresent) {
  if (!isPresent) {
    ...
    return {
      ...
      timezone: 'America/Mexico_City', evaluatedAt: '', localDate: '', opensAt: '',
      closesAt: '', nextOpenAt: '', nextCloseAt: '', nextChangeAt: '', pauseReason: '',
      pauseUntil: '', scheduleSource: 'disabled', legacy: true
    };
  }
  const value = asObject(rawAvailability);
  const code = asText(value.code);
  const timezone = asText(value.timezone);
  const valid = typeof value.acceptingOrders === 'boolean' && AVAILABILITY_CODES.has(code) && Boolean(timezone);
  return {
    ...
    localDate: /^\d{4}-\d{2}-\d{2}$/.test(asText(value.localDate)) ? asText(value.localDate) : '',
    opensAt: asText(value.opensAt), closesAt: asText(value.closesAt),
    nextOpenAt: asText(value.nextOpenAt), nextCloseAt: asText(value.nextCloseAt),
    nextChangeAt: asText(value.nextChangeAt), pauseReason: asText(value.pauseReason),
    pauseUntil: asText(value.pauseUntil),
    scheduleSource: SCHEDULE_SOURCES.has(value.scheduleSource) ? value.scheduleSource : 'missing',
    legacy: false
  };
}
...
function normalizePortalResult(data) {
  ...
  const normalizedPortal = {
    slug: asText(portal.slug), name: asText(portal.name, 'Tienda online'),
    headline: asText(portal.headline), description: asText(portal.description),
    templateCode: asText(portal.templateCode, 'classic'),
    customizationLevel: asText(portal.customizationLevel, 'basic'),
    theme: asObject(portal.theme), logoUrl: asText(portal.logoUrl),
    coverImageUrl: asText(portal.coverImageUrl), whatsappPhone: asText(portal.whatsappPhone),
    ...
    stockMode: asText(portal.stockMode, 'hidden'), settings: asObject(portal.settings)
  };
  return {
    portal: normalizedPortal,
    hours: { weekly: asArray(hours.weekly), exceptions: asArray(hours.exceptions) },
    features: normalizeFeatures(data.features),
    availability: normalizeAvailability(data.availability, normalizedPortal, Object.prototype.hasOwnProperty.call(data, 'availability')),
...
function normalizeCatalogResult(data, expectedRevision = null) {
  ...
  const items = asArray(data.items).map((rawItem) => {
    ...
    return {
      id: asText(item.id), name: asText(item.name, 'Producto'),
      description: asText(item.description), categoryName: asText(item.categoryName),
      price: Math.max(0, asNumber(item.price, 0)),
      currency: asText(item.currency, DEFAULT_CURRENCY).toUpperCase(),
      imageUrl: asText(item.imageUrl), isAvailable: asBoolean(item.isAvailable, true),
      ...
      stock: {
        ...
        quantity: stockMode === 'exact' && Number.isFinite(Number(stock.quantity))
          ? Math.max(0, Math.floor(Number(stock.quantity))) : null
      },
...
function normalizeWhatsappUrl(value) {
  ...
  try {
    ...
    if (parsed.protocol !== 'https:' || parsed.hostname !== 'wa.me' || parsed.port || parsed.username || parsed.password) return '';
...
function normalizeOrderResult(data) {
  ...
  return {
    ...
    order: {
      id: asText(order.id), code: asText(order.code), status: asText(order.status, 'new'),
      total: Number(total.toFixed(2)), currency: asText(order.currency, DEFAULT_CURRENCY).toUpperCase(),
      fulfillmentMethod: ['pickup', 'delivery'].includes(order.fulfillmentMethod) ? order.fulfillmentMethod : 'pickup',
      createdAt: asText(order.createdAt),
      trackingToken: /^trk1_[A-Za-z0-9_-]{43}$/.test(asText(order.trackingToken)) ? asText(order.trackingToken) : '',
      trackingPath: normalizeTrackingPath(order.trackingPath)
    },
    whatsapp: {
      phone: asText(whatsapp.phone), message: asText(whatsapp.message),
...
function normalizeCustomer(customer) {
  ...
  return {
    name: asText(source.name).slice(0, 120), phone: asText(source.phone).slice(0, 40),
    address: fulfillmentMethod === 'delivery' ? asText(source.address).slice(0, 500) : '',
    notes: asText(source.notes).slice(0, 1000), fulfillmentMethod
  };
}
  ...
  return asArray(items).map(buildMinimalConfiguredOrderItem).map((item) => ({
    ...
    ...(asArray(item.selections).length ? {
      selections: item.selections.map((selection) => ({
        ...
        optionIds: asArray(selection.optionIds).map(asText).filter(Boolean)
      }))
    } : {})
  }));
}
...
async function executeRpc(client, rpcName, params, operation = 'store') {
  if (!client?.rpc) throw new EcommercePublicError('ECOMMERCE_PUBLIC_CONFIG_MISSING', getSafeMessage('', operation));
  let response;
  try {
    response = await withTimeout(client.rpc(rpcName, params), {
      timeoutMessage: operation === 'checkout' ? CHECKOUT_REQUEST_MESSAGE
        : operation === 'configuration' ? CONFIGURATION_REQUEST_MESSAGE : STORE_REQUEST_MESSAGE
    });
  } catch (error) {
...
const safely = async (operation, fallback = null) => { try { return await operation(); } catch { return fallback; } };

export function createEcommercePublicService(
  client = ecommercePublicClient,
  { cache = ecommercePublicCatalogCache, configurationCache = ecommercePublicConfigurationCache } = {}
) {
  return {
    async getPublicPortalBySlug(slug, options = {}) {
      const normalizedSlug = asText(slug).toLowerCase();
      if (!normalizedSlug) throw new EcommercePublicError('ECOMMERCE_PORTAL_NOT_FOUND', 'Esta tienda no está disponible.');
      try {
    ...
    async getPublicCatalog(slug, options = {}) {
      ...
      if (cache && options.cache !== false && catalogRevision) {
        const cached = await safely(() => cache.getPage({ slug: normalizedSlug, catalogRevision, offset, limit, cachePolicy, allowStale: true }));
        if (cached) return { ...cached.page, source: 'cache', offline: options.offline === true, cacheFresh: cached.fresh, cacheAgeSeconds: cached.ageSeconds };
      }
      ...
      try {
        let data;
        try { data = await executeRpc(client, 'ecommerce_get_catalog', params); }
        catch (error) {
          if (!catalogRevision || !isLegacyCatalogSignatureError(error)) throw error;
          data = await executeRpc(client, 'ecommerce_get_catalog', { p_slug: normalizedSlug, p_limit: limit, p_offset: offset });
        }
        const result = normalizeCatalogResult(data, catalogRevision);
        if (catalogRevision && result.catalogRevision !== catalogRevision) {
          throw new EcommercePublicError('ECOMMERCE_CATALOG_REVISION_CHANGED', 'El catálogo cambió mientras se cargaba. Se actualizará automáticamente.');
        }
        if (cache && options.cache !== false && result.catalogRevision) {
          void safely(() => cache.putPage({ slug: normalizedSlug, catalogRevision: result.catalogRevision, offset, limit, cachePolicy, page: result }));
          void safely(() => cache.cleanup());
        }
        return { ...result, source: 'network', offline: false };
      } catch (error) {
        ...
        const cached = await safely(() => cache.getPage({ slug: normalizedSlug, catalogRevision, offset, limit, cachePolicy, allowStale: true }));
        if (!cached) throw error;
        return { ...cached.page, source: 'cache', offline: true, cacheFresh: cached.fresh, cacheAgeSeconds: cached.ageSeconds };
      }
    },
    ...
    async getPublicProductConfiguration(slug, options = {}) {
      ...
      if (!normalizedSlug || !productId) throw new EcommercePublicError('ECOMMERCE_PRODUCT_NOT_FOUND', 'Este producto ya no está disponible.');
      const key = configurationCache.buildKey({ slug: normalizedSlug, productId, catalogRevision, configurationVersion });
      const cached = configurationCache.get(key, { allowStale: options.offline === true });
      if (cached && (cached.fresh || options.offline === true)) return { ...cached.value, source: 'cache', offline: options.offline === true };
      if (options.offline === true) throw new EcommercePublicError('ECOMMERCE_PUBLIC_NETWORK_ERROR', 'Conéctate para confirmar las opciones vigentes de este producto.');
      return configurationCache.dedupe(key, async () => {
        try {
          ...
          if (!result.product.id || result.product.id !== productId) throw new EcommercePublicError('ECOMMERCE_CONFIGURATION_INVALID', CONFIGURATION_REQUEST_MESSAGE);
          if (catalogRevision && result.catalogRevision && result.catalogRevision !== catalogRevision) {
            throw new EcommercePublicError('ECOMMERCE_CONFIGURATION_CHANGED', 'La configuración cambió. Actualiza el catálogo.');
          }
          if (configurationVersion && result.product.configurationVersion !== configurationVersion) {
            throw new EcommercePublicError('ECOMMERCE_CONFIGURATION_CHANGED', 'La configuración cambió. Actualiza el catálogo.');
          }
          configurationCache.put(key, result);
          configurationCache.deleteObsolete({
            slug: normalizedSlug, productId,
          ...
          if (stale && options.allowStale === true) return { ...stale.value, source: 'cache', offline: true };
          throw error;
        }
      });
    },
...
export const getPublicProductConfiguration = (slug, options) => defaultService.getPublicProductConfiguration(slug, options);
...
export const ecommercePublicServiceInternals = Object.freeze({
  normalizePortalResult, normalizeAvailability, normalizeCatalogResult,
  normalizeCatalogConfiguration, normalizeOrderResult, normalizeRpcFailure,
  normalizeOrderItems, isLegacyCatalogSignatureError
});
