...
export const ECOMMERCE_PUBLIC_CACHE_SCHEMA_VERSION = 3;
...
const asPortalId = (value) => asText(value).slice(0, 80);
...
const sanitizePublicPortal = (rawPortal) => {
  ...
  return {
    ...
    contactEmail: trimText(portal.contactEmail, 254),
    address: trimText(portal.address, 500),
    addressStreet: trimText(portal.addressStreet, 160),
    addressNeighborhood: trimText(portal.addressNeighborhood, 120),
    addressMunicipality: trimText(portal.addressMunicipality, 120),
    addressState: trimText(portal.addressState, 80),
    addressPostalCode: trimText(portal.addressPostalCode, 5),
...
export const buildPublicCatalogCacheKey = ({ portalId = 'unscoped', slug, catalogRevision, offset, limit, schemaVersion }) => [
  encodeURIComponent(asPortalId(portalId).toLowerCase() || 'unscoped'),
...
export const buildPublicPortalCacheKey = ({ portalId = 'unscoped', slug, schemaVersion }) => [
  encodeURIComponent(asPortalId(portalId).toLowerCase() || 'unscoped'),
...
export const createEcommercePublicCatalogDatabase = (name = ECOMMERCE_PUBLIC_CACHE_DB_NAME) => {
  ...
  database.version(2).stores({
    pages: '&key, portalId, slug, catalogRevision, schemaVersion, offset, limit, createdAt, lastAccess, [portalId+slug], [portalId+catalogRevision]',
    portals: '&slug, portalId, catalogRevision, schemaVersion, createdAt, lastAccess, [portalId+slug]',
    portalEntries: '&key, portalId, slug, catalogRevision, schemaVersion, createdAt, lastAccess, [portalId+slug]'
  });
  return database;
};
...
export const createEcommercePublicCatalogCache = ({ database = defaultDatabase, now = () => Date.now(), maxStores = DEFAULT_MAX_STORES, maxPages = DEFAULT_MAX_PAGES } = {}) => {
  ...
  const getPage = async ({ portalId = 'unscoped', slug, catalogRevision, offset = 0, limit = 100, cachePolicy = ECOMMERCE_PUBLIC_CACHE_POLICY, allowStale = true }) => {
    const normalizedPortalId = asPortalId(portalId).toLowerCase() || 'unscoped'; const normalizedSlug = asText(slug).toLowerCase(); const revision = asRevision(catalogRevision); const policy = normalizeCachePolicy(cachePolicy);
    if (!normalizedSlug || !revision || policy.schemaVersion !== ECOMMERCE_PUBLIC_CACHE_SCHEMA_VERSION) return null;
    await ensureOpen();
    const key = buildPublicCatalogCacheKey({ portalId: normalizedPortalId, slug: normalizedSlug, catalogRevision: revision, offset, limit, schemaVersion: policy.schemaVersion });
  ...
  const putPage = async ({ portalId = 'unscoped', slug, catalogRevision, offset = 0, limit = 100, cachePolicy = ECOMMERCE_PUBLIC_CACHE_POLICY, page }) => {
    const normalizedPortalId = asPortalId(portalId).toLowerCase() || 'unscoped'; const normalizedSlug = asText(slug).toLowerCase(); const revision = asRevision(catalogRevision); const policy = normalizeCachePolicy(cachePolicy); const safePage = sanitizeCatalogPage(page, revision);
    ...
    const key = buildPublicCatalogCacheKey({ portalId: normalizedPortalId, slug: normalizedSlug, catalogRevision: revision, offset, limit, schemaVersion: policy.schemaVersion });
    await database.table('pages').put({ key, portalId: normalizedPortalId, slug: normalizedSlug, catalogRevision: revision, schemaVersion: policy.schemaVersion, offset: Math.max(0, asInteger(offset, 0)), limit: Math.min(100, Math.max(1, asInteger(limit, 100))), page: safePage, createdAt, lastAccess: createdAt });
    return true;
  };
  const getPortal = async ({ portalId, slug, maxStaleSeconds = ECOMMERCE_PUBLIC_CACHE_POLICY.maxStaleSeconds }) => {
    const normalizedPortalId = asPortalId(portalId).toLowerCase(); const normalizedSlug = asText(slug).toLowerCase(); if (!normalizedSlug) return null; await ensureOpen();
    const record = normalizedPortalId
      ? await database.table('portalEntries').where('[portalId+slug]').equals([normalizedPortalId, normalizedSlug]).first()
      : (await database.table('portalEntries').where('slug').equals(normalizedSlug).filter((candidate) => candidate.schemaVersion === ECOMMERCE_PUBLIC_CACHE_SCHEMA_VERSION).toArray()).length === 1
        ? (await database.table('portalEntries').where('slug').equals(normalizedSlug).filter((candidate) => candidate.schemaVersion === ECOMMERCE_PUBLIC_CACHE_SCHEMA_VERSION).first())
        : null;
    if (!record || record.schemaVersion !== ECOMMERCE_PUBLIC_CACHE_SCHEMA_VERSION) return null;
    const key = record.key;
    if (ageSeconds(record) > Math.max(1, asInteger(maxStaleSeconds, 86400))) { await database.table('portalEntries').delete(key); return null; }
    const result = sanitizePortalResult(record.result); if (!result) { await database.table('portalEntries').delete(key); return null; }
    void database.table('portalEntries').update(key, { lastAccess: now() }); return result;
  };
  const putPortal = async ({ portalId, slug, result }) => {
    const normalizedPortalId = asPortalId(portalId || result?.portalId).toLowerCase() || 'unscoped'; const normalizedSlug = asText(slug).toLowerCase(); const safeResult = sanitizePortalResult(result);
    if (!normalizedSlug || !safeResult || safeResult.portal.slug !== normalizedSlug || safeResult.portalId !== normalizedPortalId) return false;
    await ensureOpen(); const createdAt = now();
    const key = buildPublicPortalCacheKey({ portalId: normalizedPortalId, slug: normalizedSlug, schemaVersion: ECOMMERCE_PUBLIC_CACHE_SCHEMA_VERSION });
    await database.table('portalEntries').put({ key, portalId: normalizedPortalId, slug: normalizedSlug, catalogRevision: safeResult.catalogRevision, siteVersionId: safeResult.site.versionId || null, siteVersionNumber: safeResult.site.versionNumber || null, schemaVersion: ECOMMERCE_PUBLIC_CACHE_SCHEMA_VERSION, result: safeResult, createdAt, lastAccess: createdAt }); return true;
  };
  const deleteObsoleteRevisions = async ({ portalId = 'unscoped', slug, keepRevision }) => {
    const normalizedPortalId = asPortalId(portalId).toLowerCase() || 'unscoped'; const normalizedSlug = asText(slug).toLowerCase(); const revision = asRevision(keepRevision); if (!normalizedSlug || !revision) return 0; await ensureOpen();
    const keys = await database.table('pages').where('[portalId+slug]').equals([normalizedPortalId, normalizedSlug]).filter((record) => record.catalogRevision !== revision).primaryKeys();
  ...
  const cleanup = async () => {
    ...
    await database.table('pages').where('lastAccess').below(cutoff).delete(); await database.table('portalEntries').where('lastAccess').below(cutoff).delete();
    const portals = await database.table('portalEntries').orderBy('lastAccess').reverse().toArray();
    const allowed = new Set(portals.slice(0, maxStores).map((record) => `${record.portalId}:${record.slug}`));
    if (portals.length > maxStores) { await database.table('portalEntries').bulkDelete(portals.slice(maxStores).map((record) => record.key)); const keys = await database.table('pages').filter((record) => record.portalId && !allowed.has(`${record.portalId}:${record.slug}`)).primaryKeys(); if (keys.length) await database.table('pages').bulkDelete(keys); }
  ...
  const clear = async () => { await ensureOpen(); await database.transaction('rw', database.table('pages'), database.table('portals'), database.table('portalEntries'), async () => { await database.table('pages').clear(); await database.table('portals').clear(); await database.table('portalEntries').clear(); }); };
...
export const ecommercePublicCatalogCacheInternals = Object.freeze({
  ...
  PUBLIC_OPTION_COLLECTION_KEYS, PUBLIC_OPTION_SCALAR_KEYS, PUBLIC_SETTING_KEYS, asPortalId
});
