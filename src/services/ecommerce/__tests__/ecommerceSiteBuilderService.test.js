...
describe('ecommerceSiteBuilderService', () => {
  ...
  it('uses the shared protected context and paginates version metadata', async () => {
    ...
    expect(rpc).toHaveBeenCalledWith('ecommerce_admin_list_site_versions_v2', { ...context, p_limit: 50, p_offset: 0 });
  ...
  it('sends the exact revision and document and exposes only requested RPC flows', async () => {
    ...
    expect(rpc).toHaveBeenLastCalledWith('ecommerce_admin_save_site_draft_v2', { ...context, p_expected_revision: 7, p_document: document });
