...
begin
  ...
  -- Normalize defensively instead of returning historical JSON verbatim. Older
  -- rows may contain unknown keys or malformed values from before the builder.
  v_theme := jsonb_build_object(
    'primaryColor', case
      when jsonb_typeof(v_portal.theme) = 'object'
        and v_portal.theme->>'primaryColor' ~ '^#[0-9A-Fa-f]{6}$'
        then lower(v_portal.theme->>'primaryColor')
      else '#0284c7'
    end,
    'secondaryColor', case
      when jsonb_typeof(v_portal.theme) = 'object'
        and v_portal.theme->>'secondaryColor' ~ '^#[0-9A-Fa-f]{6}$'
        then lower(v_portal.theme->>'secondaryColor')
      else '#0369a1'
    end,
    'cornerStyle', case
      when jsonb_typeof(v_portal.theme) = 'object'
        and v_portal.theme->>'cornerStyle' in ('rounded', 'soft', 'square')
        then v_portal.theme->>'cornerStyle'
      else 'rounded'
    end,
    'fontStyle', case
      when jsonb_typeof(v_portal.theme) = 'object'
        and v_portal.theme->>'fontStyle' in ('system', 'rounded', 'editorial')
        then v_portal.theme->>'fontStyle'
      else 'system'
    end
  );
  ...
  return jsonb_build_object(
    ...
    'tracking', jsonb_strip_nulls(jsonb_build_object(
      ...
      'storefront', jsonb_build_object(
        'name', coalesce(nullif(btrim(v_portal.name), ''), 'Tienda online'),
        'logoUrl', nullif(btrim(v_portal.logo_url), ''),
        'theme', jsonb_build_object(
          'primaryColor', v_theme->>'primaryColor',
          'secondaryColor', v_theme->>'secondaryColor',
          'cornerStyle', v_theme->>'cornerStyle',
          'fontStyle', v_theme->>'fontStyle'
        )
      ),
...
comment on function public.ecommerce_get_order_tracking(text, text) is
  'ECOM.ORDERS.2.2-compatible public tracking with preserved client, portal and valid-token limits plus allowlisted storefront branding.';
