-- ECOM.ORDERS.TRACKING.BRANDING.1
-- Keep the public tracking route visually aligned with the originating storefront.
...
as $function$
declare
  v_slug text; v_token text; v_token_hash bytea;
  v_portal public.ecommerce_portals%rowtype;
  v_rate_limit jsonb; v_order public.ecommerce_orders%rowtype;
  v_tracking private.ecommerce_order_tracking_tokens%rowtype;
  v_items jsonb; v_realtime_enabled boolean := false; v_public_status text;
begin
  v_slug := lower(left(btrim(coalesce(p_slug, '')), 160));
  v_token := left(btrim(coalesce(p_tracking_token, '')), 128);
  if v_slug = '' or v_token !~ '^trk1_[A-Za-z0-9_-]{43}$' then return private.ecommerce_tracking_not_found_v1(); end if;
  ...
  v_rate_limit := public.enforce_pos_rpc_rate_limit_v2(
    ...
    p_staff_session_token := null, p_rpc_name := 'ecommerce_get_order_tracking',
    p_scope := 'ECOMMERCE_ORDER_TRACKING', p_max_attempts := 120, p_window_seconds := 600,
    p_block_seconds := 300, p_code := 'ECOMMERCE_TRACKING_RATE_LIMITED',
    p_metadata := jsonb_build_object('source', 'ecommerce_public_tracking', 'phase', 'ECOM.ORDERS.2')
  );
  if coalesce((v_rate_limit ->> 'allowed')::boolean, false) is false then return private.ecommerce_tracking_rate_limited_v1(); end if;
  ...
  if v_tracking.order_id is null or v_order.id is null then return private.ecommerce_tracking_not_found_v1(); end if;
  ...
  return jsonb_build_object('success', true, 'tracking', jsonb_strip_nulls(jsonb_build_object(
    'orderCode', v_order.public_order_code, 'status', v_public_status,
    'fulfillmentMethod', v_order.fulfillment_method, 'createdAt', v_order.created_at,
    'updatedAt', coalesce(v_order.fulfillment_updated_at, v_order.updated_at), 'total', v_order.total,
    'currency', v_order.currency, 'items', coalesce(v_items, '[]'::jsonb),
    ...
    'storefrontAvailable', v_portal.status = 'published' and v_portal.ordering_enabled,
    'storefront', jsonb_build_object('name', v_portal.name, 'logoUrl', v_portal.logo_url, 'theme', v_portal.theme),
...
exception when others then return private.ecommerce_tracking_not_found_v1();
end;
$function$;
...
comment on function public.ecommerce_get_order_tracking(text, text) is
  'Public allowlisted ecommerce order tracking lookup with safe storefront branding.';
