...
begin
  ...
  if v_order.id is null then
    v_response := jsonb_build_object('success', true, 'found', false, 'code', 'RESTAURANT_ORDER_NOT_FOUND', 'message', 'La venta se cobro, pero no habia comanda cloud abierta para cerrar.', 'idempotency_key', p_idempotency_key);
    perform private.complete_pos_idempotency(v_license_id, p_idempotency_key, v_response);
    return v_response;
  end if;

  if v_order.status = 'cancelled' then
    v_response := jsonb_build_object('success', true, 'found', true, 'code', 'ORDER_ALREADY_CANCELLED', 'message', 'La comanda ya estaba cancelada. No se reabrio ni se movio caja.', 'order', private.pos_restaurant_order_to_jsonb(v_order, null), 'idempotency_key', p_idempotency_key);
    perform private.complete_pos_idempotency(v_license_id, p_idempotency_key, v_response);
    return v_response;
  end if;
  ...
  set payment_status = 'paid',
      ...
      checkout_close_metadata = coalesce(checkout_close_metadata, '{}'::jsonb) || jsonb_build_object('phase', 'REST.7', 'closedBy', 'pos_close_restaurant_order_after_checkout', 'paymentSummary', v_payment_summary, 'paidSaleId', v_paid_sale_id, 'paidSaleFolio', v_paid_sale_folio, 'paidTotal', p_paid_total, 'statusBefore', v_order.status, 'statusAfter', v_next_status),
  ...
  v_event := private.record_pos_sync_event(v_license_id, 'restaurant_order', v_saved.id, 'close', v_device_id, v_staff_user_id, p_idempotency_key, jsonb_build_object('source', 'pos_close_restaurant_order_after_checkout', 'action', 'checkout_close', 'payment_status', 'paid', 'status_before', v_order.status, 'status_after', v_saved.status, 'paid_sale_id', v_saved.paid_sale_id, 'paid_sale_folio', v_saved.paid_sale_folio), v_saved.server_version);

  v_response := jsonb_build_object('success', true, 'found', true, 'code', case when v_saved.status = 'delivered' then 'ORDER_PAID_AND_DELIVERED' else 'ORDER_PAID_STILL_IN_KITCHEN' end, 'message', case when v_saved.status = 'delivered' then 'Comanda pagada y cerrada en cocina cloud.' else 'Comanda pagada. Cocina sigue visible hasta terminar preparacion.' end, 'order', private.pos_restaurant_order_to_jsonb(v_saved, null), 'event', to_jsonb(v_event), 'serverVersion', v_saved.server_version, 'changeSeq', v_event.change_seq, 'idempotency_key', p_idempotency_key);
