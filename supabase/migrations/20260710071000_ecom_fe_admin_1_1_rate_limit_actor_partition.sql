-- ECOM.FE.ADMIN.1.1 hardening - impedir que un dispositivo admin fragmente
-- el rate limit enviando tokens staff arbitrarios.
-- No edita la migracion ya aplicada; reemplaza de forma idempotente el helper v2.
...
begin
  ...
  -- Esta consulta solo decide la particion del rate limiter. No autoriza la RPC.
  -- Admin, roles desconocidos e intentos con credenciales no confiables comparten
  -- la particion del dispositivo; solo un dispositivo staff autenticado por su
  -- security token puede usar la particion de sesion staff.
  select d.device_role
  into v_rate_limit_device_role
  ...
  v_rate_limit := public.enforce_pos_rpc_rate_limit_v2(
    ...
    p_staff_session_token := case
      when v_rate_limit_device_role = 'staff'
        then nullif(btrim(coalesce(p_staff_session_token, '')), '')
      else null
    end,
    ...
    p_metadata := jsonb_build_object(
      ...
      'actor_partition', case
        when v_rate_limit_device_role = 'staff' then 'staff_session'
        else 'device'
      end
    )
  );
