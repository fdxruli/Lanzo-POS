-- Compensatory fix for JSON operator precedence in the foundation migration.
...
begin
  if p_document is null or jsonb_typeof(p_document) <> 'object' or exists (select 1 from jsonb_object_keys(p_document) key where key not in ('schemaVersion','global','sections')) then return 'ECOMMERCE_SITE_DOCUMENT_INVALID'; end if;
