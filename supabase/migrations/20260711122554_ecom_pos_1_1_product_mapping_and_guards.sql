-- ECOM.POS.1.1
-- Resolve every ecommerce order item against the server-owned publication.
-- The local product identifiers are text in both source columns; no casts are needed.
...
begin
  ...
  if new.published_product_id is not null
    ...
    and new.license_id is not null then
...
drop trigger if exists ecommerce_order_items_resolve_source_product_v1
  on public.ecommerce_order_items;

create trigger ecommerce_order_items_resolve_source_product_v1
before insert or update of published_product_id, portal_id, license_id
on public.ecommerce_order_items
for each row
execute function private.ecommerce_resolve_order_item_source_product_v1();
...
revoke all on function private.ecommerce_resolve_order_item_source_product_v1()
