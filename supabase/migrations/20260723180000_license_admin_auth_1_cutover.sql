...
declare v_license record; v_device record; v_has_owner boolean; v_staff_access_available boolean;
begin
  ...
  if v_has_owner then
    return json_build_object(
      'success',false,'code','ADMIN_OR_STAFF_LOGIN_REQUIRED','access_choice_required',true,
      'details',json_build_object(
        'license_key',license_key_param,'device_role','admin','plan_code',v_license.plan_code,
        'plan_name',v_license.plan_name,'features',v_license.features,
  ...
  if v_device.id is not null and v_device.is_active and v_device.device_role='admin' then
    return json_build_object(
      'success',false,'code','ADMIN_ENROLLMENT_REQUIRED','admin_enrollment_required',true,
      'details',json_build_object(
        'license_key',license_key_param,'device_role','admin','plan_code',v_license.plan_code,
        'plan_name',v_license.plan_name,'features',v_license.features,
        'staff_access_available',v_staff_access_available
  ...
  if v_device.id is not null and v_device.device_role='staff' then
    return json_build_object(
      'success',false,'code','STAFF_LOGIN_REQUIRED','staff_login_required',true,
      'details',json_build_object(
        'license_key',license_key_param,'device_role','staff','plan_code',v_license.plan_code,
        'plan_name',v_license.plan_name,'features',v_license.features,
        'staff_access_available',v_staff_access_available
      )
    );
...
create or replace function private.ecommerce_admin_authorize_v2(
  ...
  -- Historical name retained because CREATE OR REPLACE cannot rename an
  -- existing input parameter. It carries the canonical actor session token.
