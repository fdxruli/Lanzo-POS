-- HOTFIX ECOM.PRODUCTS.RECOVERY
-- Permite que una lectura confirmada y versionada recupere un producto que
-- previamente quedó marcado como source_missing sin revisión comparable.
...
begin
  ...
  -- Un source_missing es un estado técnico, no una revisión de negocio.
  -- Si reaparece una lectura confirmada con version/timestamp, puede aplicar
  -- incluso cuando el tombstone almacenado no tenía kind/order/revision.
  if p_existing_hash like 'source-missing:%' then
    if p_incoming_kind in ('version', 'timestamp')
       and p_incoming_order is not null then
