-- FASE CAD.1.1 — Migracion reproducible para bloqueo y auditoria de caducidad por lote.
-- Objetivo: reconstruir CAD.1 desde el repositorio sin depender de cambios manuales.
...
is 'CAD.1: considera vencido solo si expiry_date::date < current_date; un lote que vence hoy sigue vendible hoy.';
...
as $$
begin
  insert into public.pos_sale_audit_events (license_id, sale_id, event_type, payload)
  values (p_license_id, p_sale_id, p_event_type, coalesce(p_payload, '{}'::jsonb));
  return coalesce(p_payload, '{}'::jsonb);
exception when others then
  return coalesce(p_payload, '{}'::jsonb) || jsonb_build_object('audit_warning', sqlerrm);
end;
$$;

comment on function private.audit_pos_inventory_block(uuid,text,text,jsonb)
is 'CAD.1: registra intentos de venta bloqueados por caducidad sin romper la respuesta principal.';
...
begin
  ...
  loop
    ...
    v_uses_batches := private.product_uses_batches(v_product) or v_batch_id is not null or jsonb_array_length(v_batches_used) > 0;
    ...
    if not v_uses_batches then
    ...
    elsif jsonb_array_length(v_batches_used) > 0 then
      v_allocated := 0;
      v_available_total := 0;
      ...
      loop
        v_batch_id := private.pos_sale_jsonb_text(v_batch_payload.payload, array['batch_id','batchId','id']);
      ...
      if v_remaining > 0.00001 then
        if v_strict_expiry and v_expired_available_total > 0 then
          v_response := jsonb_build_object('ok', false, 'success', false, 'code', 'INSUFFICIENT_NON_EXPIRED_STOCK', 'message', 'No hay stock vigente suficiente para completar esta venta.', 'product_id', v_product_id, 'product_name', v_product_name, 'requested_quantity', v_quantity, 'available_quantity', v_available_total, 'expired_available_quantity', v_expired_available_total, 'source', 'resolve_sale_inventory_allocations');
...
is 'CAD.1: preflight transaccional de inventario cloud. En STRICT excluye vencidos FEFO y bloquea batch_id/batches_used vencidos.';
...
begin
  ...
  if v_batch.id is not null then
    ...
    new.metadata := coalesce(new.metadata, '{}'::jsonb)
      || jsonb_strip_nulls(jsonb_build_object('batchSku', v_batch.sku, 'batchExpiryDate', v_batch.expiry_date::date));
  end if;
...
for each row execute function private.fill_pos_sale_item_batch_snapshot();

comment on function private.fill_pos_sale_item_batch_snapshot()
is 'CAD.1: guarda snapshot de SKU y caducidad del lote en pos_sale_items.';

-- RPCs publicas CAD.1.1. Sus cuerpos completos viven en esta migracion para reconstruccion.
...
begin
  ...
  perform private.assert_pos_permission(v_context, 'products');
  ...
  v_inserted := private.insert_pos_idempotency_processing(v_license_id, v_idempotency_key, 'inventory.expiration_waste', 'product_batch', p_batch_id, md5(coalesce(p_batch_id, '') || ':' || coalesce(p_quantity::text, 'all') || ':' || coalesce(p_reason, '') || ':' || coalesce(p_notes, '')));
  ...
  select * into v_batch
  from public.pos_product_batches b
  where b.license_id = v_license_id and b.id = p_batch_id and b.deleted_at is null
  for update;
  ...
  select * into v_product
  from public.pos_products p
  where p.license_id = v_license_id and p.id = v_batch.product_id and p.deleted_at is null
  for update;
  ...
  set stock = v_new_batch_stock,
      ...
      metadata = coalesce(metadata, '{}'::jsonb) || jsonb_build_object('expirationWasteRegisteredAt', now(), 'expirationWasteReason', coalesce(p_reason, 'caducidad'), 'expirationWasteNotes', p_notes, 'expirationWasteQuantity', v_quantity)
  ...
  v_movement := private.record_pos_inventory_movement(v_license_id, v_product.id, v_batch.id, null, null, 'manual_out', v_quantity, v_previous_product_stock, v_product.stock, v_previous_batch_stock, v_new_batch_stock, v_batch.cost, coalesce(p_reason, 'caducidad'), 'manual', v_device_id, v_staff_user_id, v_actor_key, v_actor_name, v_idempotency_key, jsonb_strip_nulls(jsonb_build_object('semantic_type', 'expiry_write_off', 'reason', coalesce(p_reason, 'caducidad'), 'notes', p_notes, 'expiry_date', v_batch.expiry_date::date, 'loss_amount', v_loss_amount, 'phase', 'fase_cad_1_1')));
  ...
  perform private.record_pos_sale_audit_event(v_license_id, null, 'inventory.expiration_waste_registered', v_device_id, v_staff_user_id, v_actor_name, jsonb_build_object('product_id', v_product.id, 'product_name', v_product.name, 'batch_id', v_batch.id, 'expiry_date', v_batch.expiry_date::date, 'quantity_written_off', v_quantity, 'loss_amount', v_loss_amount, 'movement_id', v_movement.id, 'source', 'pos_register_expiration_waste'));

  v_response := jsonb_build_object('success', true, 'batch', to_jsonb(v_batch), 'product', to_jsonb(v_product), 'inventory_movement', private.pos_inventory_movement_to_jsonb(v_movement), 'quantity_written_off', v_quantity, 'loss_amount', v_loss_amount, 'idempotency_key', v_idempotency_key);
...
is 'CAD.1.1: registra merma cloud por caducidad. Staff requiere products; admin pasa por rol de dispositivo.';
...
begin
  ...
  with scored as (
    select b.id as batch_id, b.product_id, p.name as product_name, b.sku as batch_sku, b.expiry_date, b.alert_target_date, b.alert_type, b.stock, b.committed_stock,
           ...
           b.is_active, b.status, p.expiration_mode,
           case
  ...
  select jsonb_build_object('expired_active_batches', count(*) filter (where expiry_status = 'expired' and is_active is true and status = 'active'), 'upcoming_batches', count(*) filter (where expiry_status in ('expires_today','upcoming')), 'missing_expiry_strict_batches', count(*) filter (where expiry_status = 'missing' and expiration_mode = 'STRICT'), 'current_stock', coalesce(sum(available_stock) filter (where expiry_status <> 'expired'), 0), 'expired_stock', coalesce(sum(available_stock) filter (where expiry_status = 'expired'), 0), 'expired_value', coalesce(sum(stock_value) filter (where expiry_status = 'expired'), 0), 'risk_value', coalesce(sum(stock_value) filter (where expiry_status in ('expires_today','upcoming')), 0), 'days_ahead', v_days),
         coalesce(jsonb_agg(jsonb_build_object('batch_id', batch_id, 'product_id', product_id, 'product_name', product_name, 'batch_sku', batch_sku, 'expiry_date', expiry_date::date, 'alert_target_date', alert_target_date::date, 'alert_type', alert_type, 'expiry_status', expiry_status, 'stock', stock, 'committed_stock', committed_stock, 'available_stock', available_stock, 'unit_cost', unit_cost, 'stock_value', stock_value, 'is_active', is_active, 'status', status, 'expiration_mode', expiration_mode) order by expiry_date asc nulls last, product_name asc), '[]'::jsonb)
  into v_summary, v_batches
  from scored
  where expiry_status in ('expired', 'expires_today', 'upcoming') or (expiry_status = 'missing' and expiration_mode = 'STRICT');

  return jsonb_build_object('success', true, 'summary', coalesce(v_summary, '{}'::jsonb), 'batches', coalesce(v_batches, '[]'::jsonb));
end;
$$;

comment on function public.pos_get_expiring_batches_report(text,text,text,text,integer,boolean)
is 'CAD.1.1: diagnostico de caducidad por lotes. Staff requiere reports; admin pasa por rol de dispositivo.';
