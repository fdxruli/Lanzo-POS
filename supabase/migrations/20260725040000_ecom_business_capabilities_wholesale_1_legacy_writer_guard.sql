...
as $function$
begin
  if old.public_configuration_mode = 'simple_override'
     and new.public_configuration_mode = 'simple_override' then
...
drop trigger if exists ecommerce_business_capability_parent_guard
...
begin
  ...
  if v_parent.public_configuration_mode in (
    'simple_override',
    'requires_review',
    'hidden_incompatible'
  ) then
