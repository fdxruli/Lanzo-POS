...
begin
  ...
  return jsonb_build_object('success',true,'tracking',jsonb_strip_nulls(jsonb_build_object(
    ...
    'realtime',jsonb_build_object(
      ...
      'topic',case when v_realtime_enabled then private.ecommerce_tracking_topic_v1(v_tracking.token_hash) else null end
    )
  )));
exception
  when others then return private.ecommerce_tracking_not_found_v1();
end;
$function$;
