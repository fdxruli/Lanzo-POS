...
begin
  ...
  select jsonb_build_object(
    ...
    'cash_cancellation_reversals', coalesce((
      select sum(m.amount) from public.pos_cash_movements m
      where m.license_id = v_license_id and m.deleted_at is null
        and m.type in ('cancelacion_venta', 'cancelacion_abono_inicial')
        and m.created_at >= v_from and m.created_at < v_to
        and (v_staff_filter is null or m.staff_user_id = v_staff_filter)
    ), 0),
  ...
  select jsonb_build_object(
    ...
    'cloud_inventory_return_quantity', coalesce((
      select sum(r.quantity)
      from public.pos_inventory_movements r
      where r.license_id = v_license_id
        and r.deleted_at is null
        and r.movement_type = 'return_in'
        and r.source = 'sale_cancellation'
        and r.created_at >= v_from and r.created_at < v_to
        and (v_staff_filter is null or r.actor_staff_user_id = v_staff_filter)
    ), 0)
  ) into v_inventory
  from public.pos_inventory_movements m
  join public.pos_sales s on s.license_id = m.license_id and s.id = m.sale_id
  ...
  select jsonb_build_object(
    'cloud_gross_sales_total', coalesce(sum(s.total), 0),
    'cloud_sales_total', coalesce(sum(s.total), 0),
    'cloud_net_sales_total', coalesce(sum(s.total), 0),
    'cloud_sales_count', count(*)::integer,
    'cloud_cash_sales_total', coalesce(sum(coalesce(nullif(s.metadata #>> '{payment_summary,cash_component}', '')::numeric, 0)), 0),
    'cloud_non_cash_sales_total', coalesce(sum(coalesce(nullif(s.metadata #>> '{payment_summary,non_cash_component}', '')::numeric, 0)), 0),
  ...
  select jsonb_build_object(
    'cloud_cancelled_sales_count', count(*)::integer,
    'cloud_cancelled_sales_total', coalesce(sum(c.original_total), 0),
    'cloud_cash_reversal_total', coalesce(sum(c.cash_reversal_amount), 0),
    'cloud_credit_reversal_total', coalesce(sum(c.credit_reversal_amount), 0),
    'cloud_inventory_return_quantity_cancelled', coalesce(sum(c.inventory_reversal_quantity), 0)
  ) into v_cancellations
  from public.pos_sale_cancellations c
  where c.license_id = v_license_id
    and c.created_at >= v_from and c.created_at < v_to
    and (v_staff_filter is null or c.actor_staff_user_id = v_staff_filter);
  ...
  return jsonb_build_object(
    ...
    'overview', v_customers || v_cash || v_products || v_cloud_sales || v_inventory || v_cancellations,
    'source', private.reports_source_metadata('cloud') || jsonb_build_object('experimental', jsonb_build_array('Ventas cloud caja 6B', 'Inventario cloud venta 6C', 'Credito cloud venta fiada 6D', 'Cancelaciones cloud 6E')),
    'data_sources', private.reports_source_metadata('cloud') || jsonb_build_object('experimental', jsonb_build_array('Ventas cloud caja 6B', 'Inventario cloud venta 6C', 'Credito cloud venta fiada 6D', 'Cancelaciones cloud 6E')),
    'warnings', (private.reports_source_metadata('cloud')->'warnings') || jsonb_build_array('Ventas canceladas se separan de ventas netas cloud. Utilidad historica definitiva queda para Fase 6F.')
  );
end;
$$;;
