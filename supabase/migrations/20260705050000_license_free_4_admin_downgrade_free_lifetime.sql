-- FASE LICENSE.FREE.4
-- Downgrade administrativo PRO/BASIC/otro plan -> FREE permanente.
--
-- Alcance:
-- - Solo modifica public.admin_change_license_plan(text,text,text).
-- - Mantiene plans.code = 'free_trial'.
-- - Mantiene period_type = 'trial' para FREE por compatibilidad con el constraint actual.
-- - No toca frontend, create_free_trial_license, renew_license_free, caja, restaurante,
--   inventario, ventas, cloud sync ni IA.
...
begin
  ...
  -- El trigger global trg_sync_license_plan_snapshot_before_write todavia normaliza
  -- free_trial como trial / Lanzo POS (FREE-TRIAL) cuando cambia plan_id.
  -- Esta correccion es acotada al flujo administrativo hacia FREE permanente.
  if v_is_free_target then
    update public.licenses l
