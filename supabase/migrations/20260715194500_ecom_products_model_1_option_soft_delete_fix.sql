...
-- Allow an option to be soft-deleted after its parent group was soft-deleted
-- in the same atomic synchronization, without permitting active options under
-- an inactive group.
...
begin
  ...
  if tg_table_name = 'ecommerce_published_options' then
    ...
    if new.deleted_at is null and v_group.deleted_at is not null then
      raise exception 'ECOMMERCE_OPTION_GROUP_INACTIVE';
    end if;
    ...
    new.is_available := case
      when new.deleted_at is not null then false
      else new.manual_available and new.source_available
    end;
    return new;
  end if;
...
comment on function private.ecommerce_configuration_child_guard() is
  'Tenant/source guard for normalized ecommerce variants, groups and options. Supports atomic parent/child soft deletion while rejecting active options under inactive groups.';
