...
set search_path = ''
as $function$
  select coalesce(p_status in ('completed', 'cancelled'), false);
$function$;
...
begin
  if private.ecommerce_order_fulfillment_terminal_v1(old.fulfillment_status) then
    if (
      (
        new.pos_draft_status in ('claimed', 'prepared')
        and (
      ...
      or (
        new.pos_conversion_status = 'reserved'
        and (
      ...
      or (
        new.pos_conversion_status = 'completed'
        and old.pos_conversion_status is distinct from 'completed'
      )
    ) then
      raise exception using
        errcode = 'P0001',
        message = 'ECOMMERCE_ORDER_FULFILLMENT_TERMINAL';
    end if;
  end if;
...
drop trigger if exists ecommerce_orders_block_terminal_pos_mutation
...
update public.ecommerce_orders
set pos_visibility_status = 'archived',
    updated_at = now()
where fulfillment_status in ('completed', 'cancelled')
  and pos_visibility_status <> 'archived';

revoke all on function private.ecommerce_order_fulfillment_terminal_v1(text)
from public, anon, authenticated;
revoke all on function private.ecommerce_orders_block_terminal_pos_mutation_v1()
from public, anon, authenticated;
...
comment on function private.ecommerce_order_fulfillment_terminal_v1(text)
is 'ECOM.ORDERS.2.1: identifies completed/cancelled fulfillment states.';
comment on function private.ecommerce_orders_block_terminal_pos_mutation_v1()
is 'ECOM.ORDERS.2.1: defense-in-depth guard against starting POS work after terminal fulfillment.';
;
