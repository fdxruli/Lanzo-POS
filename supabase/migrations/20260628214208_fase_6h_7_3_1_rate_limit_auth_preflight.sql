...
BEGIN
  ...
  SELECT l.id, l.license_key, l.status, l.expires_at, COALESCE(p.code, l.license_type::text) AS plan_code, p.name AS plan_name, COALESCE(p.features, '{}'::jsonb) AS plan_features, COALESCE(l.features, '{}'::jsonb) AS license_features
  INTO v_license
  ...
  SELECT d.id, d.license_id, d.device_fingerprint, d.security_token, d.previous_security_token, d.is_active, COALESCE(d.device_role, 'staff') AS device_role, d.staff_user_id, d.realtime_topic
  INTO v_device
  ...
  IF v_device.device_role = 'staff' THEN
    IF v_device.staff_user_id IS NULL THEN
      RETURN jsonb_build_object('success', false, 'allowed', false, 'code', 'STAFF_LOGIN_REQUIRED', 'message', 'Inicio de sesión staff requerido.');
    END IF;
    IF NULLIF(BTRIM(COALESCE(p_staff_session_token, '')), '') IS NULL THEN
      RETURN jsonb_build_object('success', false, 'allowed', false, 'code', 'STAFF_SESSION_REQUIRED', 'message', 'Sesión staff requerida.');
    END IF;
    SELECT ss.id AS session_id, ss.expires_at, ss.revoked_at, s.id AS staff_user_id, s.username, s.display_name, s.role_name, COALESCE(s.permissions, '{}'::jsonb) AS permissions, s.is_active AS staff_is_active
    INTO v_session
    ...
    LIMIT 1;
    IF NOT FOUND THEN
      RETURN jsonb_build_object('success', false, 'allowed', false, 'code', 'STAFF_SESSION_INVALID', 'message', 'Sesión staff inválida.');
    END IF;
    IF v_session.expires_at < now() THEN
      RETURN jsonb_build_object('success', false, 'allowed', false, 'code', 'STAFF_SESSION_EXPIRED', 'message', 'Sesión staff expirada.');
    END IF;
    IF v_session.staff_is_active IS NOT TRUE THEN
      RETURN jsonb_build_object('success', false, 'allowed', false, 'code', 'STAFF_USER_INACTIVE', 'message', 'Usuario staff inactivo.');
    END IF;
    v_staff_user_id := v_session.staff_user_id;
    v_staff_permissions := COALESCE(v_session.permissions, '{}'::jsonb);
  END IF;

  RETURN jsonb_build_object('success', true, 'allowed', true, 'license_id', v_license.id, 'license_key', v_license.license_key, 'device_id', v_device.id, 'device_role', v_device.device_role, 'staff_user_id', v_staff_user_id, 'staff_permissions', v_staff_permissions, 'plan_code', v_license.plan_code, 'plan_name', v_license.plan_name, 'features', COALESCE(v_features, '{}'::jsonb), 'realtime_topic', v_device.realtime_topic);
END;
$$;
...
BEGIN
  ...
  LOOP
    ...
    IF COALESCE(v_arg_names[1], '') <> 'p_license_key'
      ...
      RAISE EXCEPTION 'FASE 6H.7.3.1: firma inesperada en %. Primeros args: %, %, %, %', v_unlimited_name, COALESCE(v_arg_names[1], '<null>'), COALESCE(v_arg_names[2], '<null>'), COALESCE(v_arg_names[3], '<null>'), COALESCE(v_arg_names[4], '<null>');
    END IF;
    SELECT pg_get_function_identity_arguments(v_unlimited_oid), pg_get_function_arguments(v_unlimited_oid)
    INTO v_identity_args, v_args_with_defaults;
    SELECT string_agg('$' || gs::text, ', ' ORDER BY gs)
    INTO v_arg_refs
    FROM generate_series(1, (SELECT p.pronargs FROM pg_proc p WHERE p.oid = v_unlimited_oid)) AS gs;
    EXECUTE format($wrapper$
      ...
      BEGIN
        ...
        IF COALESCE((v_context->>'success')::boolean, false) IS FALSE THEN
          RETURN v_context;
        END IF;
        v_rate_limit := public.enforce_pos_rpc_rate_limit($1, $2, $4, %3$L, 60, %4$s, %5$s);
      ...
      END;
      $fn$;
    $wrapper$, v_spec.rpc_name, v_args_with_defaults, v_spec.rpc_name, v_spec.device_max_requests, COALESCE(v_spec.license_max_requests::text, 'NULL'), v_unlimited_name, v_arg_refs);
    EXECUTE format('GRANT EXECUTE ON FUNCTION public.%I(%s) TO anon, authenticated', v_spec.rpc_name, v_identity_args);
    EXECUTE format('REVOKE ALL ON FUNCTION public.%I(%s) FROM PUBLIC, anon, authenticated', v_unlimited_name, v_identity_args);
  END LOOP;
END;
$$;
