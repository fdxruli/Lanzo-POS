-- FASE LICENSE.FREE.2.1
-- Correccion defensiva dentro de la misma etapa LICENSE.FREE.2.
--
-- Motivo:
-- trg_sync_license_plan_snapshot_before_write normaliza cualquier INSERT con plan
-- free_trial a license_type = 'trial' y product_name = 'Lanzo POS (FREE-TRIAL)'.
-- Para no tocar el trigger global, ni el downgrade PRO -> FREE, ni licencias existentes,
-- esta RPC deja que el trigger haga su snapshot y despues restaura los campos FREE
-- solo para la licencia nueva creada desde onboarding.
...
begin
  ...
  loop
    ...
    begin
      ...
      -- El trigger de snapshot global reescribe free_trial como trial en INSERT.
      -- Se corrige solo esta licencia recien creada, sin modificar el trigger ni otros flujos.
