...
-- Preserve omitted image properties, honour explicit JSON null, and keep slug_source authoritative.
...
declare
  ...
  v_name text; v_slug text; v_requested_slug text; v_status text; v_whatsapp text; v_pickup boolean; v_delivery boolean; v_min_order numeric(12,2);
...
begin
  ...
  if v_custom_slug_allowed and v_requested_slug is not null then
    v_slug := v_requested_slug; if length(v_slug) not between 3 and 64 or v_slug !~ '^[a-z0-9][a-z0-9-]*[a-z0-9]$' then return private.ecommerce_admin_error('ECOMMERCE_SLUG_INVALID'); end if;
  elsif not v_custom_slug_allowed and v_existing.id is not null and v_requested_slug is not null and v_requested_slug <> v_existing.slug then
  ...
  begin
    ...
    -- JSON property absence preserves; JSON null explicitly unlinks; only HTTPS is accepted for SET.
  ...
  values (v_license_id,v_slug,case when v_custom_slug_allowed and v_requested_slug is not null then 'custom' else 'system' end,v_status,v_name,nullif(btrim(p_payload->>'headline'),''),nullif(btrim(p_payload->>'description'),''),v_template,case when v_advanced_branding then 'advanced' else 'basic' end,v_theme,v_logo,v_cover,nullif(v_whatsapp,''),nullif(btrim(p_payload->>'address'),''),v_pickup,v_delivery,v_min_order,'hidden',coalesce(p_payload->'settings','{}'::jsonb),jsonb_build_object('source','admin_ui','phase','HOTFIX.ECOM.PORTAL.CUSTOMIZATION.1.1'))
  on conflict (license_id) where deleted_at is null do update set
    slug=excluded.slug,
    -- Replaying the current slug is not a customization. Preserve the stored source in that case.
    slug_source=case when v_custom_slug_allowed and v_requested_slug is not null and v_requested_slug <> ecommerce_portals.slug then 'custom' else ecommerce_portals.slug_source end,
    status=excluded.status,name=excluded.name,headline=excluded.headline,description=excluded.description,template_code=excluded.template_code,customization_level=excluded.customization_level,theme=excluded.theme,logo_url=excluded.logo_url,cover_image_url=excluded.cover_image_url,whatsapp_phone=excluded.whatsapp_phone,address=excluded.address,pickup_enabled=excluded.pickup_enabled,delivery_enabled=excluded.delivery_enabled,min_order_total=excluded.min_order_total,
    stock_mode=case when coalesce((v_features->>'ecommerce_stock_visibility')::boolean,false) then ecommerce_portals.stock_mode else 'hidden' end,
    settings=coalesce(p_payload->'settings',ecommerce_portals.settings),metadata=coalesce(ecommerce_portals.metadata,'{}'::jsonb)||jsonb_build_object('last_admin_source','admin_ui','phase','HOTFIX.ECOM.PORTAL.CUSTOMIZATION.1.1')
