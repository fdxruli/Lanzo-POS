-- ECOM.PORTAL.BUILDER.1 hardening. This migration is intentionally not a rewrite
-- of the four already-deployed migrations.
...
alter table public.ecommerce_site_documents
  add column document_mode text not null default 'default'
    check (document_mode in ('default', 'custom'));

-- Existing BUILDER.1 rows only contain generated presets; future structural
-- editing explicitly marks a document custom when it no longer equals the preset.
...
-- Check the encoded size before inspecting the shape.  This makes the limit
-- effective even for malicious payloads that are otherwise invalid.
...
begin
  ...
  if exists (select 1 from jsonb_object_keys(p_document) key where key not in ('schemaVersion','global','sections')) then return 'ECOMMERCE_SITE_DOCUMENT_INVALID'; end if;
  if (p_document->>'schemaVersion') <> '1' then return 'ECOMMERCE_SITE_SCHEMA_UNSUPPORTED'; end if;
  if jsonb_typeof(p_document->'global') <> 'object' or exists (select 1 from jsonb_object_keys(p_document->'global') key where key not in ('themeSource','contentWidth','density')) or (p_document #>> '{global,themeSource}') <> 'portal' or (p_document #>> '{global,contentWidth}') <> 'standard' or (p_document #>> '{global,density}') not in ('comfortable','compact') then return 'ECOMMERCE_SITE_DOCUMENT_INVALID'; end if;
  ...
  for v_section in select value from jsonb_array_elements(p_document->'sections') loop
    if jsonb_typeof(v_section) <> 'object' or exists (select 1 from jsonb_object_keys(v_section) key where key not in ('id','type','enabled','layout','props','style')) or (v_section->>'id') !~ '^[a-z][a-z0-9-]{2,63}$' or jsonb_typeof(v_section->'enabled') <> 'boolean' or jsonb_typeof(v_section->'props') <> 'object' or (v_section ? 'style' and (jsonb_typeof(v_section->'style') <> 'object' or v_section->'style' <> '{}'::jsonb)) then return 'ECOMMERCE_SITE_SECTION_INVALID'; end if;
    ...
    if (v_type='header' and ((v_section->>'layout') not in ('default','showcase') or v_props <> jsonb_build_object('contentSource','portal')))
      or (v_type='catalog' and ((v_section->>'layout') not in ('grid','compact') or exists (select 1 from jsonb_object_keys(v_props) key where key not in ('showSearch','showCategories')) or jsonb_typeof(v_props->'showSearch') <> 'boolean' or jsonb_typeof(v_props->'showCategories') <> 'boolean'))
...
returns trigger language plpgsql security definer set search_path to '' as $$
begin
  if current_setting('app.ecommerce_site_version_insert', true) <> 'authorized' then
...
begin
  ...
  if v_document.document_mode = 'default' and v_document.draft_document <> v_default then
    update public.ecommerce_site_documents set draft_document=v_default,draft_revision=draft_revision+1,updated_at=now() where portal_id=v_portal.id returning * into v_document;
  end if;
  return v_document;
end;
$$;
...
declare v_auth jsonb; v_doc public.ecommerce_site_documents%rowtype; v_error text; v_template text;
begin
  ...
  select template_code into v_template from public.ecommerce_portals where id=v_doc.portal_id;
  update public.ecommerce_site_documents set draft_document=p_document,document_mode=case when p_document=private.ecommerce_site_default_document(v_template) then 'default' else 'custom' end,draft_revision=draft_revision+1,updated_at=now(),updated_by_device_id=(v_auth->>'device_id')::uuid,updated_by_staff_user_id=nullif(v_auth->>'staff_user_id','')::uuid where portal_id=v_doc.portal_id returning * into v_doc;
...
create function public.ecommerce_admin_list_site_versions(p_license_key text,p_device_fingerprint text,p_security_token text,p_staff_session_token text,p_limit integer default 20,p_offset integer default 0)
returns jsonb language plpgsql security definer set search_path to '' as $$
declare v_auth jsonb; v_doc public.ecommerce_site_documents%rowtype; v_limit integer:=least(greatest(coalesce(p_limit,20),1),50); v_offset integer:=greatest(coalesce(p_offset,0),0);
begin
  v_auth:=private.ecommerce_site_authorize(p_license_key,p_device_fingerprint,p_security_token,p_staff_session_token,'ecommerce_admin_list_site_versions'); if coalesce((v_auth->>'success')::boolean,false) is not true then return v_auth; end if;
  v_doc:=private.ecommerce_site_document_for_auth(v_auth);
  return jsonb_build_object('success',true,'limit',v_limit,'offset',v_offset,'hasMore',exists(select 1 from public.ecommerce_site_versions where portal_id=v_doc.portal_id offset (v_offset + v_limit) limit 1),'versions',coalesce((select jsonb_agg(jsonb_build_object('id',id,'versionNumber',version_number,'schemaVersion',schema_version,'documentChecksum',document_checksum,'source',source,'createdAt',created_at,'restoredFromVersionId',restored_from_version_id) order by version_number desc) from (select * from public.ecommerce_site_versions where portal_id=v_doc.portal_id order by version_number desc limit v_limit offset v_offset) versions),'[]'::jsonb));
exception when others then return private.ecommerce_admin_error('ECOMMERCE_SITE_ACCESS_DENIED'); end;
$$;
...
returns jsonb language plpgsql security definer set search_path to '' as $$
declare v_auth jsonb; v_doc public.ecommerce_site_documents%rowtype; v_version public.ecommerce_site_versions%rowtype; v_template text;
begin
  v_auth:=private.ecommerce_site_authorize(p_license_key,p_device_fingerprint,p_security_token,p_staff_session_token,'ecommerce_admin_restore_site_version'); if coalesce((v_auth->>'success')::boolean,false) is not true then return v_auth; end if;
  v_doc:=private.ecommerce_site_document_for_auth(v_auth,true); select * into v_version from public.ecommerce_site_versions where id=p_version_id and portal_id=v_doc.portal_id; if v_version.id is null then return private.ecommerce_admin_error('ECOMMERCE_SITE_VERSION_NOT_FOUND'); end if;
  select template_code into v_template from public.ecommerce_portals where id=v_doc.portal_id;
  update public.ecommerce_site_documents set draft_document=v_version.document,document_mode=case when v_version.document=private.ecommerce_site_default_document(v_template) then 'default' else 'custom' end,draft_revision=draft_revision+1,updated_at=now(),updated_by_device_id=(v_auth->>'device_id')::uuid,updated_by_staff_user_id=nullif(v_auth->>'staff_user_id','')::uuid where portal_id=v_doc.portal_id returning * into v_doc;
  return jsonb_build_object('success',true,'draft',jsonb_build_object('document',v_doc.draft_document,'revision',v_doc.draft_revision,'updatedAt',v_doc.updated_at,'documentMode',v_doc.document_mode));
exception when others then return private.ecommerce_admin_error('ECOMMERCE_SITE_SAVE_FAILED'); end;
$$;
...
returns jsonb language plpgsql security definer set search_path to '' as $$
declare v_portal public.ecommerce_portals%rowtype; v_version public.ecommerce_site_versions%rowtype; v_document jsonb; v_error text; v_document_mode text := 'default';
begin
