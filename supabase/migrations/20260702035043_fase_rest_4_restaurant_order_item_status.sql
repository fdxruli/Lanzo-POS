...
-- FASE REST.4 — Estados por item en comandas cloud y cierre operativo por estación.
-- Idempotente, no destructiva. No toca caja, cobro, stock, venta final ni impresión.

alter table public.pos_restaurant_order_items
  add column if not exists started_at timestamptz null,
  add column if not exists ready_at timestamptz null,
  add column if not exists delivered_at timestamptz null,
  add column if not exists cancelled_at timestamptz null,
  add column if not exists updated_by_device_id uuid null references public.license_devices(id) on delete set null,
  add column if not exists updated_by_staff_user_id uuid null references public.license_staff_users(id) on delete set null;

create index if not exists pos_restaurant_order_items_status_lookup_idx
  on public.pos_restaurant_order_items (license_id, restaurant_order_id, status)
  where deleted_at is null;

create index if not exists pos_restaurant_order_items_station_status_lookup_idx
  on public.pos_restaurant_order_items (license_id, station_code, status, updated_at desc)
  where deleted_at is null;
...
returns jsonb language sql stable set search_path to '' as $$
  select jsonb_build_object(
    ...
    'updatedByStaffUserId', p_row.updated_by_staff_user_id,
...
begin
  ...
  into v_total, v_preparing, v_done
  ...
  set status = v_next_status,
      ...
      metadata = coalesce(metadata, '{}'::jsonb) || jsonb_build_object('phase', 'REST.4', 'statusRecalculatedBy', 'private.recalculate_restaurant_order_status')
  where license_id = p_license_id
    and id = p_restaurant_order_id
    and deleted_at is null
...
begin
  ...
  set status = v_status,
      ...
      updated_at = now(),
      server_version = server_version + 1,
      metadata = coalesce(metadata, '{}'::jsonb) || jsonb_build_object('phase', 'REST.4', 'statusUpdatedBy', 'pos_update_restaurant_order_item_status')
  where license_id = v_license_id
    and restaurant_order_id = p_restaurant_order_id
    and id = p_restaurant_order_item_id
    and deleted_at is null
