-- ECOM.FE.ADMIN.1.1 - Permitir administracion ecommerce a staff con sesion valida
-- y permisos vigentes settings + ecommerce, sin elevar el dispositivo a admin.
-- Mantiene intactas las firmas antiguas (admin-only) para compatibilidad.
...
begin
  ...
  v_rate_limit := public.enforce_pos_rpc_rate_limit_v2(
    ...
    p_staff_session_token := nullif(btrim(coalesce(p_staff_session_token, '')), ''),
    ...
    p_metadata := jsonb_build_object('phase', 'ECOM.FE.ADMIN.1.1')
  );
...
begin
  v_auth := private.ecommerce_admin_authorize_v2(
    ...
    p_staff_session_token := p_staff_session_token,
    p_rpc_name := 'ecommerce_admin_get_portal'
  );
...
begin
  v_auth := private.ecommerce_admin_authorize_v2(
    ...
    p_security_token := p_security_token,
    p_staff_session_token := p_staff_session_token,
    p_rpc_name := 'ecommerce_admin_upsert_portal'
  );
  if coalesce((v_auth->>'success')::boolean, false) is false then return v_auth; end if;
  ...
  if v_pickup is false and v_delivery is false then
  ...
  if v_custom_slug_allowed then
    ...
    elsif length(v_requested_slug) between 3 and 64
          and v_requested_slug ~ '^[a-z0-9][a-z0-9-]*[a-z0-9]$' then
  ...
  if v_existing.id is null then
    insert into public.ecommerce_portals (
      ...
      case when v_advanced_branding
        then coalesce(nullif(btrim(p_payload->>'templateCode'), ''), 'classic')
        else 'classic'
      end,
      ...
      jsonb_build_object('source', 'admin_ui', 'phase', 'ECOM.FE.ADMIN.1.1')
    )
    ...
    set slug = v_slug,
        slug_source = case
          when v_custom_slug_allowed and v_requested_slug <> '' then 'custom'
          else p.slug_source
        end,
        status = v_status,
        name = v_name,
        ...
        template_code = case
          when v_advanced_branding
            then coalesce(nullif(btrim(p_payload->>'templateCode'), ''), p.template_code)
          else 'classic'
        end,
        customization_level = case when v_advanced_branding then 'advanced' else 'basic' end,
        theme = case
          when v_advanced_branding then coalesce(p_payload->'theme', p.theme)
          else '{}'::jsonb
        end,
        logo_url = coalesce(nullif(btrim(p_payload->>'logoUrl'), ''), p.logo_url),
        cover_image_url = case
          when v_advanced_branding
            then coalesce(nullif(btrim(p_payload->>'coverImageUrl'), ''), p.cover_image_url)
          else null
        end,
        ...
        metadata = coalesce(p.metadata, '{}'::jsonb)
          || jsonb_build_object(
            'last_admin_source', 'admin_ui',
            'phase', 'ECOM.FE.ADMIN.1.1'
          )
...
begin
  v_auth := private.ecommerce_admin_authorize_v2(
    ...
    p_security_token := p_security_token,
    p_staff_session_token := p_staff_session_token,
    p_rpc_name := 'ecommerce_admin_list_published_products'
  );
  if coalesce((v_auth->>'success')::boolean, false) is false then return v_auth; end if;
  ...
  select
    coalesce(
      jsonb_agg(private.ecommerce_admin_product_jsonb(pp) order by pp.display_order, pp.public_name),
      '[]'::jsonb
    ),
...
begin
  ...
  v_source_type := lower(
    btrim(coalesce(p_payload->>'sourceType', coalesce(v_existing.source_type, 'local_snapshot')))
  );

  if v_source_type not in ('local_snapshot', 'cloud_product') then
    return private.ecommerce_admin_error(
      'ECOMMERCE_PRODUCT_SOURCE_INVALID',
      'La fuente del producto no es valida.'
    );
  end if;

  if v_source_type = 'cloud_product'
     and coalesce((v_features->>'ecommerce_cloud_catalog_source')::boolean, false) is false then
    return private.ecommerce_admin_error('ECOMMERCE_CLOUD_CATALOG_REQUIRES_PRO');
  end if;

  v_local_ref := nullif(
    btrim(coalesce(p_payload->>'localProductRef', coalesce(v_existing.local_product_ref, ''))),
    ''
  );
  v_cloud_ref := nullif(
    btrim(coalesce(p_payload->>'productId', coalesce(v_existing.product_id, ''))),
    ''
  );

  if v_source_type = 'local_snapshot' and v_local_ref is null then
    return private.ecommerce_admin_error('ECOMMERCE_LOCAL_PRODUCT_REF_REQUIRED');
  end if;

  if v_source_type = 'cloud_product' and v_cloud_ref is null then
    return private.ecommerce_admin_error(
      'ECOMMERCE_CLOUD_PRODUCT_REF_REQUIRED',
      'Selecciona un producto del catalogo cloud.'
    );
  end if;
  ...
  if v_existing.id is null then
    ...
    where pp.portal_id = v_portal_id
      ...
      and (
        (
          v_source_type = 'local_snapshot'
          and pp.source_type = 'local_snapshot'
          and pp.local_product_ref = v_local_ref
        )
        or (
          v_source_type = 'cloud_product'
          and pp.source_type = 'cloud_product'
          and pp.product_id = v_cloud_ref
        )
      )
    limit 1 for update;
  end if;
  ...
  if v_name = '' then
    return private.ecommerce_admin_error('ECOMMERCE_PRODUCT_NAME_REQUIRED');
  end if;
  ...
  v_is_published := coalesce(
    (p_payload->>'isPublished')::boolean,
    coalesce(v_existing.is_published, true)
  );
  v_stock_mode := lower(
    btrim(coalesce(p_payload->>'stockMode', coalesce(v_existing.stock_mode, 'hidden')))
  );
  ...
  if v_existing.id is null then
    insert into public.ecommerce_published_products (
      ...
      v_local_ref,
      v_name,
      nullif(btrim(p_payload->>'publicDescription'), ''),
      nullif(btrim(p_payload->>'categoryName'), ''),
      v_price,
      'MXN',
      nullif(btrim(p_payload->>'imageUrl'), ''),
      v_is_published,
    ...
    set source_type = v_source_type,
        ...
        display_order = greatest(
          coalesce(nullif(p_payload->>'displayOrder', '')::integer, pp.display_order),
          0
        ),
  ...
  return jsonb_build_object(
    'success', true,
    'message', case
      when v_existing.id is null then 'Producto publicado correctamente.'
...
exception
  when others then
    ...
    if sqlerrm like '%ECOMMERCE_STOCK_VISIBILITY_REQUIRES_PRO%' then
      return private.ecommerce_admin_error('ECOMMERCE_STOCK_VISIBILITY_REQUIRES_PRO');
    end if;
...
begin
  ...
  return jsonb_build_object(
    'success', true,
    'message', case
      when v_saved.is_published then 'Producto publicado.'
      else 'Producto retirado del portal.'
    end,
    'product', private.ecommerce_admin_product_jsonb(v_saved)
  );
exception
