create or replace function public.begin_ai_agent_analysis(
  ...
  p_agent_type text default 'unknown',
...
begin
  ...
  if coalesce(v_device.device_role, 'admin') = 'staff' then
    ...
    if v_staff_session.session_id is null
       or v_staff_session.revoked_at is not null
       or v_staff_session.expires_at < now()
       or v_staff_session.staff_is_active = false then
  ...
  if v_used_count >= v_limit then
    return jsonb_build_object(
      ...
      'message', 'Ya se alcanzó el límite total de análisis de IA para esta licencia.',
      'limit', v_limit,
      'used', v_used_count,
      'remaining', 0
    );
  ...
  insert into public.ai_agent_usage (
    ...
    'reserved',
    coalesce(p_metadata, '{}'::jsonb) || jsonb_build_object('plan_code', v_license.plan_code, 'plan_name', v_license.plan_name)
...
create or replace function public.complete_ai_agent_analysis(
  p_usage_id uuid,
  p_success boolean,
...
begin
  ...
  if v_row.id is null then
    return jsonb_build_object('success', false, 'code', 'USAGE_NOT_FOUND', 'message', 'Registro de uso no encontrado.');
  end if;

  return jsonb_build_object('success', true, 'usage_id', v_row.id, 'status', v_row.status);
end;
$$;;
