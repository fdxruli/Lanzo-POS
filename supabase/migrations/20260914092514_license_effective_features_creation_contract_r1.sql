-- LIC.AUTH.4.1 - Return the same effective feature contract used by verify.
-- The license row keeps only its license-owned snapshot/defaults; callers must
-- receive plan features merged with explicit license features.
