...
begin
  ...
  insert into public.license_devices(
    ...
    (v_device_c, v_license_id, v_fingerprint_c, 'Second admin station', v_device_token_c, true, 'admin', null);
  ...
  begin
    perform public.pos_create_cloud_sale_cashier(
      ...
      v_device_token_b,
      v_admin_token_b,
      jsonb_build_object('id', 'cash-multi-cross-sale-' || v_suffix, 'local_sale_id', 'cash-multi-cross-sale-' || v_suffix, 'total', 30, 'subtotal', 30, 'amount_paid', 30, 'payment_method', 'cash'),
      jsonb_build_array(jsonb_build_object('id', 'cash-multi-cross-sale-item-' || v_suffix, 'product_id', 'cash-multi-product-' || v_suffix, 'product_name', 'Multi station product', 'quantity', 1, 'unit_price', 30, 'line_subtotal', 30, 'line_total', 30)),
      jsonb_build_array(jsonb_build_object('id', 'cash-multi-cross-sale-payment-' || v_suffix, 'method', 'cash', 'amount', 30, 'received_amount', 30, 'change_amount', 0)),
      v_session_a,
      'cash-multi-cross-sale-idem-' || v_suffix
    );
