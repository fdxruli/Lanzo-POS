-- HOTFIX ECOM.OPERATIONS.1.IDEMPOTENCY regression matrix.
-- Derived from the OPERATIONS.1 transaction fixture and adds retries with changed pickup,
-- invalid payload and ordering-disabled state. All rows are rolled back.
...
declare
  ...
  v_initial_tracking_token text;
  v_initial_tracking_path text;
...
begin
  ...
  v_initial_order_id := (v_result->'order'->>'id')::uuid;
  v_initial_tracking_token := v_result->'order'->>'trackingToken';
  v_initial_tracking_path := v_result->'order'->>'trackingPath';
  select count(*) into v_initial_item_count from public.ecommerce_order_items where order_id = v_initial_order_id;
  if v_initial_order_id is null or coalesce(v_initial_tracking_token, '') = '' or coalesce(v_initial_tracking_path, '') = '' then
    raise exception 'HOTFIX_INITIAL_ORDER_CONTRACT_FAILED: %', v_result;
  end if;

  update public.ecommerce_portals set pickup_enabled = false where id = v_portal_id;
  v_result := public.ecommerce_create_order(
    'ecom-operations-1-synthetic',
    jsonb_build_object('name', 'Cliente prueba', 'phone', '9610000000', 'fulfillmentMethod', 'pickup'),
    jsonb_build_array(jsonb_build_object('productId', v_product_id, 'quantity', 1)),
  ...
  if coalesce((v_result->>'success')::boolean, false) is not true
     ...
     or (v_result->'order'->>'id')::uuid <> v_initial_order_id
     or v_result->'order'->>'trackingToken' <> v_initial_tracking_token
     or v_result->'order'->>'trackingPath' <> v_initial_tracking_path then
    raise exception 'HOTFIX_PICKUP_DISABLED_RETRY_FAILED: %', v_result;
  end if;

  v_result := public.ecommerce_create_order(
    'ecom-operations-1-synthetic',
    jsonb_build_object('name', '', 'phone', 'x', 'fulfillmentMethod', 'invalid'),
    '[]'::jsonb,
    'ecom-operations-1-idempotent'
  );
  if coalesce((v_result->>'success')::boolean, false) is not true
     or coalesce((v_result->>'idempotent')::boolean, false) is not true
     or (v_result->'order'->>'id')::uuid <> v_initial_order_id then
    raise exception 'HOTFIX_INVALID_PAYLOAD_RETRY_FAILED: %', v_result;
  end if;
  if (select count(*) from public.ecommerce_orders where portal_id = v_portal_id and idempotency_key = 'ecom-operations-1-idempotent') <> 1
     or (select count(*) from public.ecommerce_order_items where order_id = v_initial_order_id) <> v_initial_item_count then
    raise exception 'HOTFIX_RETRY_DUPLICATED_ROWS_FAILED';
  end if;
  update public.ecommerce_portals set pickup_enabled = true where id = v_portal_id;
  ...
  if coalesce((v_result->>'success')::boolean, false) is not true
     or coalesce((v_result->>'idempotent')::boolean, false) is not true
     or (v_result->'order'->>'id')::uuid <> v_initial_order_id then
    raise exception 'HOTFIX_CLOSED_RETRY_FAILED: %', v_result;
  end if;
  ...
  if coalesce((v_result->>'success')::boolean, false) is not true
     or coalesce((v_result->>'idempotent')::boolean, false) is not true
     or (v_result->'order'->>'id')::uuid <> v_initial_order_id then
    raise exception 'HOTFIX_ORDERING_DISABLED_RETRY_FAILED: %', v_result;
  end if;
