...
begin
  ...
  if has_function_privilege('anon', 'public.ecommerce_admin_upsert_portal(text,text,text,jsonb)', 'EXECUTE')
     or has_function_privilege('authenticated', 'public.ecommerce_admin_upsert_portal(text,text,text,jsonb)', 'EXECUTE') then
    raise exception 'legacy upsert overload remains publicly executable';
  end if;
  ...
  if not has_function_privilege('anon', 'public.ecommerce_admin_upsert_portal(text,text,text,text,jsonb)', 'EXECUTE')
    or not has_function_privilege('authenticated', 'public.ecommerce_admin_upsert_portal(text,text,text,text,jsonb)', 'EXECUTE') then
